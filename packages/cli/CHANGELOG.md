...
- e6d0c55: Improve the local ProofKit MCP / FileMaker setup flow during webviewer init:
  ...
  - Require an explicit local FileMaker file selection in non-interactive multi-file setups, and persist the selection (or the lone connected file) into the generated `proofkit-typegen.config.jsonc` as `fmMcp.connectedFileName`.
  - Normalize non-interactive FileMaker layout names against live layout casing so case-only drift doesn't break hosted init or release smoke tests.
