...
- 69fd3fb: **BREAKING:** Use an fmodata `Database` instance instead of a raw OData config. Config now requires `database` (a fmodata `Database`) instead of `odata: { serverUrl, auth, database }`. Enables fetch overrides via `FMServerConnection`'s `fetchClientOptions`. Adapter and migration types are also made resilient to upstream Better Auth changes.
- 2cddedf: Upgrade Better Auth peer to 1.5.x (`createAdapter` → `createAdapterFactory`, removed `getAdapter`).
