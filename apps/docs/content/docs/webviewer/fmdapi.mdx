...
Default batching uses an 8 ms window and maximum batch size of 20. Use `batch: false` on the adapter or an individual request to opt out. Benchmark your own script path before assuming batching improves speed; local `Execute FileMaker Data API` calls may see little benefit compared with server-side script execution.
