...
`WebViewerAdapter` can coalesce Data API calls that happen within a short window into one FileMaker script call. See [Batching Data API Requests](/docs/webviewer/batching) for the full script contract and older add-on fallback behavior.
