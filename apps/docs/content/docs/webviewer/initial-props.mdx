...
- **Your web app needs to know the current user at startup** &mdash; account name, display name, or privilege set so the UI can adapt before the first render.
...
- **Data you inject into the Web Viewer HTML keeps getting lost** &mdash; substitution or script-trigger approaches race against bundle loading and silently fail.
