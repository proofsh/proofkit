...
| `timeoutMs`    | `60000`                | Stops the promise hanging when no script answers. `0` waits indefinitely.       |
...
Else
  # Capture the error after each step; a later step resets Get ( LastError )
