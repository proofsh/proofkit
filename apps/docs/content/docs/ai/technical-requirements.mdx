...
ProofKit v2 is designed for FileMaker developers using agentic coding tools to build WebViewer apps.
...
After a WebViewer app is bundled and deployed into FileMaker, it can run in:

- FileMaker Pro on macOS and Windows.
- FileMaker Go on iOS and iPadOS.
- FileMaker WebDirect.

See [Platform Notes](/docs/hybrid-apps/platform-notes) for runtime caveats.

## Out of scope for v2

ProofKit v2 enables agentic coding of WebViewer apps. It does not currently enable agentic editing of:
...
| Agent writing scope | WebViewer app code |
